...
import { Zap, TrendingUp, BarChart2, Map, ArrowRight, Layers } from "lucide-react";
  ...
  return (
    <div style={{ padding: "48px 56px", maxWidth: 1200 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1, background: "#E5DFD8", border: "1px solid #E5DFD8", borderRadius: 4, overflow: "hidden", marginBottom: 40 }}>
        <KPI label="Biens en vente"  value={stats?.nb_biens_actifs ?? "—"}                                                           unit="annonces actives"       href="/map" />
        <KPI label="Leads DPE"       value={dpeStats?.total_leads ?? "—"}                                                            unit="propriétaires ciblés"   href="/dpe" />
        <KPI label="Prix médian"     value={prix?.prix_median_m2 ? `${Math.round(prix.prix_median_m2).toLocaleString("fr-FR")}` : "—"} unit="€ / m² (médian)"       href="/dvf" />
        <KPI label="Agences actives" value={stats?.nb_agences ?? "—"}                                                               unit="concurrents identifiés" href="/market" />
      </div>

      {/* Modules */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 32 }}>
        <ModuleCard icon={Zap}      eyebrow="Prospection vendeurs"  title="Radar DPE"         description="Identifiez les propriétaires en obligation de rénovation et anticipez les mises en vente."  stat={dpeStats?.urgents_count} statLabel="contacts urgents" statColor="#C0392B" href="/dpe" />
        <ModuleCard icon={TrendingUp} eyebrow="Analyse de prix"    title="Prix DVF"           description="Suivez l'évolution des prix de vente au m² par type de bien sur votre zone."                  stat={prix?.prix_median_m2 ? `${Math.round(prix.prix_median_m2).toLocaleString("fr-FR")} €` : null} statLabel="médian / m²" statColor="#B8965A" href="/dvf" />
        <ModuleCard icon={BarChart2} eyebrow="Veille concurrentielle" title="Analyse de marché" description="Biens stagnants, baisses de prix récentes et parts de marché par agence."                   stat={stats?.biens_stagnants_count} statLabel="biens stagnants" statColor="#C8810A" href="/market" />
        <ModuleCard icon={Layers}   eyebrow="Données foncières"    title="Cadastre"           description="Explorez les parcelles cadastrales, sections et surfaces foncières disponibles."              stat={null} statLabel="" statColor="#1B2A4A" href="/cadastre" />
      </div>
...
function KPI({ label, value, unit, href }: { label: string; value: string | number; unit: string; href: string }) {
  return (
    ...
    >
      <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9C8F83", fontFamily: "Inter, sans-serif", marginBottom: 12 }}>{label}</div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9C8F83", fontFamily: "Inter, sans-serif", marginBottom: 8 }}>{label}</div>
