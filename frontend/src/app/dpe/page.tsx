  ...
  return (
    <div style={{ padding: "48px 56px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {CLASSES.map((c) => (
                <button key={c} onClick={() => toggle(c)} className={`dpe-badge dpe-${c}`}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[30, 60, 90, 180].map((j) => (
                <button key={j} onClick={() => setJours(j)} style={{ padding: "4px 10px", borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "Inter, sans-serif", transition: "all 0.15s", border: "1px solid", borderColor: jours === j ? "#1B2A4A" : "#E5DFD8", background: jours === j ? "#1B2A4A" : "#FFFFFF", color: jours === j ? "#FFFFFF" : "#6B6057" }}>
                  {j}j
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginLeft: "auto" }}>
            ...
            <input type="range" min="0" max="1" step="0.1" value={score} onChange={(e) => setScore(parseFloat(e.target.value))} style={{ width: 90, accentColor: "#B8965A" }} />
            <span style={{ fontFamily: "JetBrains Mono, monospace", fontWeight: 600, fontSize: 13, color: "#B8965A", minWidth: 36 }}>{Math.round(score * 100)}%</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
        {[
          { color: "#C0392B", title: "Priorité maximale", desc: "DPE F/G · Interdit à la location dès 2025" },
          { color: "#C8810A", title: "Priorité haute",    desc: "DPE E · Loi Climat 2028" },
          { color: "#059669", title: "À surveiller",      desc: "DPE C/D · Probabilité modérée" },
        ].map((l, i) => (
      ...
      {isLoading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[...Array(6)].map((_, i) => <div key={i} className="skeleton" style={{ height: 56, borderRadius: 4 }} />)}
        </div>
      ) : !data?.leads?.length ? (
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          <table className="data-table">
            <thead><tr><th>DPE</th><th>Adresse</th><th>Surface</th><th>Diagnostic</th><th>Score vente</th><th>Priorité</th><th></th></tr></thead>
            <tbody>{data.leads.map((lead: DPELead) => <LeadRow key={lead.id} lead={lead} />)}</tbody>
          </table>
        </div>
      )}
    </div>
...
function LeadRow({ lead }: { lead: DPELead }) {
  ...
  const [color, tagClass] = s >= 5 ? ["#C0392B", "tag-red"] : s >= 4 ? ["#C8810A", "tag-amber"] : ["#059669", "tag-green"];
  ...
  return (
    <tr style={{ borderLeft: `3px solid ${color}` }}>
      ...
      <td>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 3, fontSize: 11.5, color: "#9C8F83" }}><MapPin size={10} />{lead.commune} {lead.code_postal}</div>
      </td>
      <td><span style={{ fontFamily: "JetBrains Mono, monospace", fontWeight: 600, fontSize: 13, color: "#3D3429" }}>{lead.surface_habitable ? `${lead.surface_habitable} m²` : "—"}</span></td>
      <td><span style={{ fontSize: 12, color: "#6B6057" }}>{lead.date_etablissement ? format(parseISO(lead.date_etablissement), "dd MMM yyyy", { locale: fr }) : "—"}</span></td>
      <td>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div className="progress" style={{ width: 72 }}><div className="progress-fill" style={{ width: `${pct}%`, background: color }} /></div>
      ...
      <td style={{ textAlign: "right" }}><button className="btn btn-ghost btn-sm"><ArrowUpRight size={13} /></button></td>
    </tr>
  );
}
